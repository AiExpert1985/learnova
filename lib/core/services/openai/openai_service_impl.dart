...
/// Concrete implementation of OpenAI API integration
/// Uses GPT-4 mini for cost-effective Q&A
class OpenAIServiceImpl implements OpenAIService {
  ...
  Future<OpenAIResponse> askQuestion({
    ...
    if (apiKey.isEmpty) {
      throw OpenAIException('API key is empty');
    }
...
Provide a clear, concise answer (2-3 sentences). If the answer isn't in the transcript, say "I don't have enough information from this video to answer that."''';
  ...
  void _handleErrorResponse(http.Response response) {
    ...
    throw OpenAIException(message, statusCode: statusCode);
  }

  OpenAIResponse _parseResponse(http.Response response) {
    final Map<String, dynamic> data;

    try {
      data = jsonDecode(response.body);
    } catch (e) {
      throw OpenAIException('Failed to parse response: $e');
    }

    final choices = data['choices'] as List?;
    if (choices == null || choices.isEmpty) {
      throw OpenAIException('No response from AI');
    }
    ...
    if (text == null || text.isEmpty) {
      throw OpenAIException('Empty response from AI');
    }
